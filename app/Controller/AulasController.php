...
class AulasController extends AppController {
	...
	public function verAulas() {
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'Aula.id';
		...
		if(isset($entrada)) {
			$row['total'] = $this->Aula->find('count', array('conditions' => array('Aula.nombre like' => "%$entrada%")));
			$row['rows'] = $this->Aula->find('all', array('conditions' => array('Aula.nombre like' => "%$entrada%")));
		}
		else {
			$row['total'] = $this->Aula->find('count')."";
			$row['rows'] = $this->Aula->find('all', array('order' => $sort.' '.$order, 'limit' => $offset.','.$rows, 'recursive' => -1));
                        
		}
		$this->set(compact('row["rows"]'));
		$this->set('result',$row);
		$this->render('verAulas','ajax');
	}
	...
	public function agregar() {
		...
		if ($this->request->is('post')) {
			...
			if(!empty($this->request->data)) {
                                ...
                                $existente = $this->Aula->find('count', array('conditions' => array('nombre' => $this->request->data['nombre'], 'ubicacion' => $this->request->data['ubicacion'])));

                                if($existente>0){
                                     $reg['msg'] = 'Disculpe, el aula ya se encuentra registrada!!';
                                }
                                else {
                                    //Asigna el resultado de la consulta
                                    $row = $this->Aula->save($this->request->data);
                                    if($row) {
                                            $reg['success'] = true;
	...
	public function eliminar($id) {
	  ...
	  $this->set('aulas', $this->Aula->find('all'));
      //$this->Aulas->id = $id;
