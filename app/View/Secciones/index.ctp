<script type="text/javascript">
		var url;
		function nuevo(){
			$('#dlg').dialog('open').dialog('setTitle','Nueva Sección');			
			$('#fm').form('clear');
			url = 'agregar';
		...
		function editar(){
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Editar Sección');
				$('#fm').form('load',row);
				url = 'editar/'+row.id;
			}
			else {
...
<table id="dg" title="Secciones" class="easyui-datagrid" style="height:610px" url="verSecciones"  toolbar="#toolbar"  pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" data-options="pageSize:20">  
    <thead>  
        <tr>  
...
<div id="dlg" class="easyui-dialog" style="width:280px;height:150px;padding:10px 20px; "
			closed="true">		
