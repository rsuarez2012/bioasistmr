<!--<script type="text/javascript">
		var url;
		function nuevo(){
			$('#dlg').dialog('open').dialog('setTitle','Nuevo Profesor(a)');			
			$('#fm').form('clear');	
			url = 'agregar';	
		}
		...
		function editar(){
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Editar Profesor(a)');
				$('#fm').form('load',row);
				url = 'editar/'+row.id;
			}
			else {
...
<script type="text/javascript">
		var url;
		function nuevo(){
			$('#dlg').dialog('open').dialog('setTitle','Nuevo Profesor');			
			$('#fm').form('clear');	
			url = 'agregar';	
		}
		...
		function editar(){
			var row = $('#dg').datagrid('getSelected');					
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Editar Profesor');
				$('#fm').form('load',row);
				url = 'editar/'+row.id;
			}
			else {
...
<table id="dg" title="Profesores" class="easyui-datagrid" style="height:610px" url="verPersonal"  toolbar="#toolbar"  pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" data-options="pageSize:20">  
    <thead>  
        <tr>          	
...
<div id="dlg" class="easyui-dialog" style="width:380px;height:350px;padding:10px 20px;" closed="true">		
