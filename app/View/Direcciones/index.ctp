<script type="text/javascript">
		var url;
		function nuevo(){
			$('#dlg').dialog('open').dialog('setTitle','Nueva Dirección');			
			$('#fm').form('clear');
			url = 'agregar';
		...
		function editar(){
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Editar Dirección');
				$('#fm').form('load',row);
				url = 'editar/'+row.id;
			}
			else {
...
<table id="dg" title="Aulas" class="easyui-datagrid" style="height:610px" url="verDirecciones"  toolbar="#toolbar"  pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" data-options="pageSize:20">  
    <thead>  
        <tr>  
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:200px;padding:10px 20px; "
		...
		<table>							
			<tr>
				...
				<td><?php echo $this->Form->textarea('nombre', array('div' => false, 'name' => 'nombre', 'class' => 'easyui-validatebox', 'required' => 'true', 'label' => false)); ?></td>
			</tr>			
		</table>					
