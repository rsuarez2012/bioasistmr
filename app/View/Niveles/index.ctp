<script type="text/javascript">
		var url;
		function nuevo(){
			$('#dlg').dialog('open').dialog('setTitle','Nuevo Semestre');			
			$('#fm').form('clear');
			url = 'agregar';
		...
		function editar(){
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Editar Semestre');
				$('#fm').form('load',row);
				url = 'editar/'+row.id;
			}
			else {
...
<table id="dg" title="Semestres" class="easyui-datagrid" style="height:610px" url="verNiveles"  toolbar="#toolbar"  pagination="true" rownumbers="true" fitColumns="true" singleSelect="true">  
    <thead>  
        <tr>  
            <th field="nombre" width="20" sortable="true">Nombre</th>                       
            <th field="etiqueta" width="90" sortable="true">Etiqueta</th>
        </tr>          
    </thead>  
</table>  
<div id="toolbar"> 
...
<div id="dlg" class="easyui-dialog" style="width:280px;height:180px;padding:10px 20px; "
		...
		<table>						
			...
			<tr>
				...
				<td><?php echo $this->Form->input('etiqueta', array('div' => false,'name' => 'etiqueta', 'class' => 'easyui-validatebox', 'required' => 'true', 'label' => false)); ?></td>
			</tr>							
		</table>					
